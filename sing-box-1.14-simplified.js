// Sub-Store producer script for sing-box 1.14.
//
// 1.14 differs from the 1.12/1.13 template in two ways this script must respect:
//   1. The DNS "resolve domestic IPs via dns_direct" step is now expressed with an
//      `evaluate` rule followed by a `match_response` route rule (legacy implicit
...
if (noDoh) {
  ...
  //    In 1.14 this also covers the new `evaluate` and `match_response` rules,
  //    which both carry server: dns_direct.
